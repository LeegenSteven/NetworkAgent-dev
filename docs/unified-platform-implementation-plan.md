...
Local 部署入口（2026-08-30 初始完成，2026-08-31 增加 S4-04 冷备恢复与 S4-05 runtime-trace 证据）：新增
...
| Local 部署、CI 与可观测性 | IN PROGRESS | 2026-08-31 | `local-stack` 安全本地入口及 S2-01..03 已通过；S2-04 因没有兼容、provenance 可追溯且完整 `C/H=0/0` 的基础镜像而为 `BLOCKED`。S4-01..05 窄切片均已由各自精确绑定 RC 和独立 artifact 验收并标记 `DONE`：依次交付有界阶段事件/报告内诊断、双分支只读 Canonical 投影、固定三窗口 acceptance SLI、stopped-writer Local DuckDB cold backup/restore，以及单事件单进程 Replay -> durable readback -> A2A Analyze 六事件相关性。五个窄切片合计仍未交付跨进程/分布式或 MCP/OTel 全链路 trace、OpenTelemetry/Collector、Prometheus、外部告警、时间型/延迟型/长期统计可靠性 SLO、在线/异地/加密签名/多副本/跨版本/Cloud/生产恢复或完整故障演练；S2/Workflow B、S4/Workflow E/P7/S7 保持 `IN PROGRESS`，Gate B/G2、Gate A/S3/G4、Gate E/G5、统一 UI 与 Cloud 部署保持开放。 |
