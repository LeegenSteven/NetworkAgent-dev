...
from telco_cloud_mcp import __main__ as entrypoint
...
def test_spanner_database_role_is_forwarded_without_credentials(monkeypatch) -> None:
    ...
    profile = SimpleNamespace(
        ...
        database_role="telco_mcp_reader",
        emulator_host=None,
    )
    ...
    incidents, telemetry = entrypoint._repositories()

    assert incidents._database is database
    assert telemetry._database is database
    assert calls == {
        "project": "project-demo",
        "instance": "instance-demo",
        "database": ("database-demo", "telco_mcp_reader"),
    }
...
def test_production_rejects_any_role_other_than_mcp_reader(monkeypatch) -> None:
    profile = SimpleNamespace(
        project_id="project-demo",
        instance_id="instance-demo",
        database_id="database-demo",
