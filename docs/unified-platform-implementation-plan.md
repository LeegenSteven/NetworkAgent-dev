...
> 当前里程碑：P0、P1、P2 已完成 / 实施开发计划 2.0 的 Sprint 1 Governance HTTP + Loopback Replay 已通过本地独立 Gate 与同一提交远程 RC；Sprint 2 安全容器/Compose 保持 `IN PROGRESS`，S2-01 安全容器基线、S2-02 容器成功/失败治理重启恢复与 S2-03 Trivy 双报告 runner-local release evidence 已分别在精确绑定 RC 的远程 Docker workflow 上通过并标记 `DONE`，S2-04 为 `BLOCKED`，Gate B/G2、Gate A/S3/G4 仍开放；S4-01 本地答辩可观测证据、S4-02 Canonical 生命周期安全投影、S4-03 固定三窗口 Local acceptance SLI/SLO、S4-04 Local DuckDB 冷备恢复、S4-05 Local 单进程运行时 Trace 与 S7-03 BubbleRAN 四分支 fixture 答辩入口窄切片为 `DONE`，但 S4/Workflow E/P7/S7 保持 `IN PROGRESS` 且 Gate E/G5 仍开放；P3 Cloud 代码与远程 Emulator Gate 已通过但仍等待 Cloud Staging；P3e BubbleRAN 已完成锁定下载、适配、离线评估、受控回放、caller-owned 持久 checkpoint、持久 Canonical Fault 接收、本地治理 E2E、RC 制品证据与独立代码生成 fixture 答辩入口，但仍停留于每 source 独立 Incident；P3e-5/P3e 因完整上游答辩、跨事件聚合、RCAEval 第二路径与完整发布终验未完成而保持 `IN PROGRESS`；Cloud Resolver、真实动作和统一 UI 仍待开发
...
- **P3e（IN PROGRESS）**：新增本地开放数据实验室。现有 P3c 已由只读 Cloud MCP 占用，因此开放数据工作正式编号为 P3e，不复用 P3c。当前已完成安全 catalog/lock、许可证据与归属锁定、显式许可下载、BubbleRAN 精确 schema CSV/JSON 适配、候选有界的离线评估、immutable label-free `ReplayPlan`、公开 `ReplayWirePayload` 与 S7-03 独立代码生成 fixture 答辩入口。loopback transport 现支持立即投递与单调 paced runner，只对显式启用的 network/timeout 瞬时失败做有限重试，并保留 deadline/cancel 证据；可选 caller-owned store 以严格 plan-bound JSON、原子替换和非阻塞单-writer 锁持久化 checkpoint。Assurance `POST /local/v1/faults/replay` 只在 Canonical Incident/source association 持久后返回 202；每 source 独立 Incident，精确锁定的 5G SA BubbleRAN `ran.mac.ul_bler > 0.15` 只使用服务端规则版本和内容摘要作 provenance，不得外推生产。S7-03 fixture 不是完整上游 benchmark；RCAEval 第二路径与多源/跨事件聚合仍未实现，P3e-5/P3e 不因此关闭。
...
| 本地开放数据实验室（P3e） | IN PROGRESS | 2026-08-31 | 已交付 `telco-lab` Python/CLI、稳定 lock ID、许可/来源指纹、BubbleRAN 固定 catalog、失败关闭投影、离线评估、immutable `ReplayPlan`、公开 wire、loopback/paced transport、caller-owned 持久 checkpoint、durable receiver 与真实 TCP 四终态 E2E。S7-03 另完成 4 条 `CODE_GENERATED_SCHEMA_FIXTURE` 的独立四分支答辩入口：checkpoint `4/4/4 -> 0/0/0`、Action/Verification `2/2`、绕 checkpoint 重送四类业务 delta 0。它不是完整上游 benchmark；P3e-5/P3e 仍因完整上游答辩、跨事件聚合、RCAEval 第二路径、签名/attestation 与发布终验未完成而不标 DONE。 |
...
| Local 部署、CI 与可观测性 | IN PROGRESS | 2026-08-31 | `local-stack` 安全本地入口及 S2-01..03 已通过；S2-04 为 `BLOCKED`。S4-01..05 与 S7-03 窄切片均由各自精确绑定 RC 和独立 artifact 验收：除五个 S4 证据外，S7-03 增加代码生成 fixture 的四分支真实 loopback/checkpoint/零放大答辩入口。它们合计仍未交付跨进程/分布式或 MCP/OTel 全链路 trace、Prometheus、外部告警、时间型/延迟型/长期统计可靠性 SLO、在线/异地/加密签名/多副本/跨版本/Cloud/生产恢复、完整故障演练、RCAEval 第二路径或统一 UI；S2/Workflow B、S4/Workflow E/P7/S7 保持 `IN PROGRESS`，Gate B/G2、Gate A/S3/G4、Gate E/G5 与 Cloud 部署保持开放。 |
