...
class EngineerAgentError(Exception):
    """Base exception class for Engineer Agent errors."""
...
class ToolError(EngineerAgentError):
...
class PlanningError(EngineerAgentError):
    """Exception raised for errors in the planning phase."""
    pass

class ExecutionError(EngineerAgentError):
    """Exception raised for errors in the execution phase."""
    pass

class AuthenticationError(EngineerAgentError):
...
def handle_exception(func):
    ...
    error response that can be propagated through the agent_executor interface.
    ...
    async def wrapper(*args, **kwargs):
        try:
            ...
            if e.severity == ErrorSeverity.INFO:
                logger.info(f"EngineerAgentError: {e.message}", exc_info=True)
            elif e.severity == ErrorSeverity.WARNING:
                logger.warning(f"EngineerAgentError: {e.message}", exc_info=True)
            elif e.severity == ErrorSeverity.ERROR:
                logger.error(f"EngineerAgentError: {e.message}", exc_info=True)
            elif e.severity == ErrorSeverity.CRITICAL:
                logger.critical(f"EngineerAgentError: {e.message}", exc_info=True)
            
            # Re-raise the error to be handled by the caller
            raise
        except Exception as e:
            # Convert generic exceptions to EngineerAgentError
            error = EngineerAgentError(
            ...
            # Re-raise as EngineerAgentError
            raise error
