...
class GanttChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final rootSpanId in rootSpanOrder) {
      ...
      for (int i = 0; i < group.length; i++) {
        ...
        final double eventStart = event.startTime.difference(groupStart).inMilliseconds * pixelsPerMs;
        final double eventDuration = event.endTime.difference(event.startTime).inMilliseconds * pixelsPerMs;
        
        final double left = currentX + indentation + eventStart;
        final double width = eventDuration.clamp(5.0, double.infinity); // Minimum 5px width
      ...
      final groupWidth = (groupDuration * pixelsPerMs).clamp(200.0, double.infinity);
      ...
      if (textPainter.width + (textPadding * 2) <= rect.width) {
        ...
        final outsideTextPainter = TextPainter(
          text: TextSpan(
            text: labelText,
            style: TextStyle(
              color: event.eventColor, // Use event color since it's on white background
              fontSize: 12,
              fontWeight: FontWeight.bold,
            ),
          ),
  ...
  String _findRootSpanId(ProcessedTraceEvent event, List<ProcessedTraceEvent> allEvents) {
