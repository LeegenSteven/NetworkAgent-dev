...
Sprint 1 远程回归绑定 release candidate `427fc6832bf6b115d035e5d2cb492a25ffd82395`；下列四个成功 run 的 `headSha` 均精确等于该 SHA：

- [Assurance CI run 33296728012](https://github.com/LeegenSteven/NetworkAgent-dev/actions/runs/33296728012)：Python 3.12/3.13 各通过 Domain 323、Lab 197、Local 195、Lab E2E 1（另 1 skipped）、Local E2E 3、Assurance 50、A2A contracts 33、A2A E2E 4，四 wheel、源码树外 smoke 和 `pip check` 全绿。
- [Data Lab CI run 33296728022](https://github.com/LeegenSteven/NetworkAgent-dev/actions/runs/33296728022)：Python 3.12/3.13 + Pydantic 2.13.4 各 `198 passed, 1 skipped`，Python 3.12 + 声明下限 Pydantic 2.5.3 也为 `198 passed, 1 skipped`，wheel 内容 allowlist、源码树外 smoke 和 `pip check` 全绿。
