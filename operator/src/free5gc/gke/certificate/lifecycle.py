...
from free5gc.gke.certificate.lifecycle_tasks import *
...
async def creategitcertificate(spec, **_):
  ...
  if cluster.get('status') is None:
    raise kopf.TemporaryError("No Cluster status", 30)

  status = cluster.get("status").get("conditions")[0]
  logger.debug(status)
  if status is None or status.get("reason") != "UpToDate":
    raise kopf.TemporaryError("Waiting for cluster to be ready", 30)

  # create namespace and git secret in the new cluster
  api_client= get_api_client(cluster.status.endpoint, cluster.status.observedState.masterAuth.clusterCaCertificate)
