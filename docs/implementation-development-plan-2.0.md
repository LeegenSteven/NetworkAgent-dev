...
> 建立日期：2026-08-30
> 最近更新：2026-08-31
> 当前执行阶段：S4 — 可观测与运维（`IN PROGRESS`；S4-01、S4-02、S4-03、S4-04、S4-05、S7-03、S7-04 与 S7-05 窄切片 `DONE`，S2-04 仍为 `BLOCKED`）
...
| S7-04 RCAEval 五案例回答盲离线评估 | `DONE` | RC `b8a9e958a0a3354634f87e2fbc8f76aaf60913dd` 的 Lab/Assurance/Container push runs 与显式 Lab dispatch 全绿；16 个锁定资源、五个 aggregate-only case、pre-reveal seal/batch commitment、post-reveal 原 seal 复用、隐私输出及离线 exact result 均通过。唯一 Lab artifact 9755569487 已通过 10 文件闭包、canonical summary、wheel/SBOM/audit 复核。该窄切片不是完整 benchmark、生产准确率、泛化/因果、RCAEval Replay、在线/Cloud/OTel/UI 或 P3e/S7/Gate 关闭。 |
| S7-05 离线提交证据索引包 | `DONE` | RC `67f08d887d6ba6e5ddb4108603af71b19e5e0d0f` 的 Submission 双 Python 构建、跨版本摘要相等和 clean rebuild/publish 全绿；唯一 artifact 9763040737 已通过 8,759-byte raw ZIP、exact-five/CRC/manifest、4/4 bytes/SHA、9 slices/24 runs/66 jobs/9 citations、CSP/隐私/非闭环独立复核。该窄切片只验收 builder/workflow/bundle，不重跑或嵌入九个历史切片，也不改变其 RC 身份。 |
...
| Local Open Data Lab | `IN PROGRESS` | BubbleRAN 下载锁定、隐私投影、离线评估、受控回放与独立 fixture 答辩入口，以及 RCAEval 五案例锁定上游、aggregate-only 适配、回答盲排序/评估与离线复验均已完成；P3e 仍因 RCAEval Replay、完整上游 benchmark、跨事件聚合、通用 Gate 与完整发布终验未完成而保持进行中。 |
...
- S7-04 的唯一受测 release candidate 为 `b8a9e958a0a3354634f87e2fbc8f76aaf60913dd`；[Data Lab push run 33385845017](https://github.com/LeegenSteven/NetworkAgent-dev/actions/runs/33385845017)、[Assurance push run 33385845041](https://github.com/LeegenSteven/NetworkAgent-dev/actions/runs/33385845041) 与 [Container push run 33385844990](https://github.com/LeegenSteven/NetworkAgent-dev/actions/runs/33385844990) 均为 `success`。显式 [Data Lab dispatch 33385881296](https://github.com/LeegenSteven/NetworkAgent-dev/actions/runs/33385881296) 的 Python 3.12 [job 99468272496](https://github.com/LeegenSteven/NetworkAgent-dev/actions/runs/33385881296/job/99468272496)、Pydantic [job 99468272632](https://github.com/LeegenSteven/NetworkAgent-dev/actions/runs/33385881296/job/99468272632) 与 Python 3.13 [job 99468272707](https://github.com/LeegenSteven/NetworkAgent-dev/actions/runs/33385881296/job/99468272707) 全绿。
- 固定 RCAEval 路径锁定 revision `afeacb11bcc94dadfd1c8f483ee4377b2b8b614e`、16 resources / 53,433,532 bytes、closure `c99ced28f1cb56464820a9570ead783de753c31ad36f5d7d29de594115101fb1` 与 MIT evidence `c2990bbe2e040a8d2f55fdd47c4f47f02223d8ea098e5d6e8851585a64956a0f`。五个 case 在答案加载前全部 rank/seal 并建立 batch commitment，揭示后验证同一 commitment 并复用原 seals；5 ranked / 0 inconclusive，AC@1..5、average@5、MRR 均为 1,000,000 ppm，ownership validity 为 104,838 ppm（39/372 references）。这些指标只属于该五案例固定切片。
- 唯一 S7-04 制品为 Python 3.12 Data Lab [VERIFIED RC artifact 9755569487](https://github.com/LeegenSteven/NetworkAgent-dev/actions/runs/33385881296/artifacts/9755569487)，名称 `telco-lab-release-py3.12-attempt-1`，148,959 bytes，archive SHA-256 `8afc11102a17310c78e1295a15a758396d904c9aea964985801c0e9e30fd88f4`，创建/到期时间为 `2026-08-31T11:13:40Z` / `2026-09-14T11:13:40Z`。独立下载确认 10 文件闭包（9 records + manifest）、9/9 bytes/SHA 与 manifest `PASS`；2,408-byte `rcaeval-upstream-summary.json` 的 SHA-256 为 `999a35e25bfa53aaf3ef7f86f7eaf4b596c17b25366ba85cf7193724a41d0b38` 且 canonical bytes 精确一致。两个 wheels / 47 members 不含 Parquet/Arrow/Feather/IPC/ORC/CSV/DuckDB/JSONL；runtime inventory 为 2 first-party + 6 runtime，CycloneDX 1.4 为 8 components 并含 PyArrow 25.0.0，`pip-audit` 为 0，wheel scan 为 `PASS`。
- 本次 S7-04 文档提交晚于且不等于受测 RC。该结果只关闭 S7-04 五案例回答盲离线评估窄切片，不关闭 S7/P7/P3e/S4/Workflow E 或 Gate E/G5/G2/G4，也不改变 S2-04 `BLOCKED` 或 P6 `NOT STARTED`；不声明完整 benchmark、生产准确率、泛化、因果、live remediation、online、externally timestamped、Cloud/OTel/dashboard 或 RCAEval Replay。
- S7-05 的唯一受测 release candidate 为 `67f08d887d6ba6e5ddb4108603af71b19e5e0d0f`。[telco-submission run 33405685328](https://github.com/LeegenSteven/NetworkAgent-dev/actions/runs/33405685328) 为 `success` / 33s：Python 3.12 [job 99532578035](https://github.com/LeegenSteven/NetworkAgent-dev/actions/runs/33405685328/job/99532578035) 为 7s / 9 success steps，Python 3.13 [job 99532578383](https://github.com/LeegenSteven/NetworkAgent-dev/actions/runs/33405685328/job/99532578383) 为 10s / 9，cross equality [job 99532644051](https://github.com/LeegenSteven/NetworkAgent-dev/actions/runs/33405685328/job/99532644051) 为 3s / 3，clean rebuild/publish [job 99532668880](https://github.com/LeegenSteven/NetworkAgent-dev/actions/runs/33405685328/job/99532668880) 为 11s / 10。
- 同一 push 实际还触发 [telco-local run 33405685291](https://github.com/LeegenSteven/NetworkAgent-dev/actions/runs/33405685291)，状态 `success` / 12m19s；Python 3.12 job 99532577952 为 12m14s / 21 success steps，Python 3.13 job 99532578269 为 12m15s / 17 success steps，四项发布步骤按设计跳过。该 Local 回归不把九个历史窄切片重写为本 RC 的重新验收。
- Submission 仅 Python 3.12 发布保留 14 天的 [artifact 9763040737](https://github.com/LeegenSteven/NetworkAgent-dev/actions/runs/33405685328/artifacts/9763040737)，名称 `networkagent-local-submission-py3.12-attempt-1`；Python 3.13 和 pull request 均不上传。raw ZIP 实测 8,759 bytes，outer SHA-256 为 `12e626cd7125f6b04b9f8d817f2db7abadc3c8eba4fca7068909e61355d53295`，inner manifest SHA-256 为 `32a2ffafaab603cd222c6fadf8e4111ecedb26b354d24d8409520c007c4d4e5c`。独立下载确认 exact-five unique regular entries、CRC、四条 manifest records 加 manifest 自身、4/4 bytes/SHA；index/limitations SHA-256 分别为 `741f4cfb29c992e88eaaba0887cdc11373ae536f1683bc86280e6ce5d3594260` / `dd0ca40daeca8d004dafddfee673403f9750bb67a781528721cddd5ec39403f7`，9 slices/24 runs/66 jobs/9 historical artifacts、CSP/四链接、隐私、非闭环与引用政策均通过。
- 本次 S7-05 文档提交晚于且不等于受测 RC。该 RC 只验收 builder、`telco-submission` workflow 与五文件 bundle；九个历史切片仍绑定各自 `rc_sha`/runs/jobs/artifacts，未被重跑，payload 未嵌入，持续远程可用性也未声明。只关闭 S7-05 窄切片；S7/P7/P3e/S4/Workflow E 继续 `IN PROGRESS`，P6 为 `NOT STARTED`，S2-04 为 `BLOCKED`，Gate E/G2/G4/G5 保持开放。
...
- S7-04 已由精确绑定 RC `b8a9e958a0a3354634f87e2fbc8f76aaf60913dd` 的三个 push workflows、显式 Lab 三 job dispatch 与唯一 artifact 9755569487 独立闭包验收，因此仅“RCAEval 五案例锁定上游回答盲离线评估”窄切片为 `DONE`。它不提供完整 benchmark/全数据覆盖、上游实现等价、生产准确率、泛化/因果、在线评估、RCAEval Replay、外部时间戳、Cloud/OTel/UI 或 Gate 关闭；P3e 与 S7 总体保持 `IN PROGRESS`。
- S7-05 已由精确绑定 RC `67f08d887d6ba6e5ddb4108603af71b19e5e0d0f` 的双 Python 构建、跨版本 equality Gate、clean rebuild/publish、唯一 artifact 9763040737 与独立 exact-five 下载闭包验收，因此仅“九个历史窄切片的离线安全索引包”标记 `DONE`。它不重跑历史验收、不嵌 payload、不保证历史 artifact 仍可下载，也不关闭 S7/P7/P3e/S4/Workflow E 或任何开放 Gate。
...
| S7 答辩发布 | 固化版本、证据包、演示脚本、限制声明 | A–F | `IN PROGRESS` | S7-01、S7-02、S7-03、S7-04、S7-05 与支持答辩的 S4-01、S4-02、S4-03、S4-04、S4-05 窄切片均已标记 `DONE`；S7-03 只覆盖 BubbleRAN 代码生成 fixture 四分支入口，S7-04 只覆盖 RCAEval 五案例回答盲离线评估，S7-05 只覆盖九个历史切片元数据与限制的五文件离线索引包。完整 S4/Workflow E、P3e、RCAEval Replay/完整 benchmark、最终阶段限制声明及 G2/G4/G5 等适用 Gate 仍未通过。 |
...
### 6.9 S7-04 RCAEval 五案例回答盲离线评估

**状态**：`DONE`（仅限本节定义的窄切片）。

唯一入口为 `telco-lab --workspace .local/networkagent-rcaeval run rcaeval-re2ob-multisource-rca --accept-license MIT`。它锁定 16 个真实上游资源 / 53,433,532 bytes，在只使用 label-free timing 与 aggregate-only features 的阶段完成五个 rank/seal；私有 slot 不进入 ranker。随后用 catalog/lock/closure/case digests/features/seals 建立 batch commitment，创建后才揭示答案；post-reveal 必须验证同一 commitment 并复用原 seals，最后才构造私有 truth mapping 并评估。输出省略 private sample、candidate、reference identifier、artifact location 与 raw row。

固定五案例得到 5 ranked / 0 inconclusive，AC@1..5、Average@5 与 MRR 均为 1,000,000 ppm；ownership validity 为 104,838 ppm，即 39 个 truth-owned references / 372 个 ranked references。上述分数只属于锁定五案例，ownership validity 也不是准确率或独立证据标注质量。离线 `verify` 加 `evaluate` 必须在 16/16 closure 上重建与首次 `run` 逐结构相等的 result。

受测 RC `b8a9e958a0a3354634f87e2fbc8f76aaf60913dd` 的 Data Lab/Assurance/Container push runs 33385845017/33385845041/33385844990 与显式 Data Lab dispatch 33385881296 全绿；显式 dispatch jobs 为 99468272496/99468272632/99468272707。唯一 Python 3.12 [VERIFIED RC artifact 9755569487](https://github.com/LeegenSteven/NetworkAgent-dev/actions/runs/33385881296/artifacts/9755569487) 为 148,959 bytes / archive SHA-256 `8afc11102a17310c78e1295a15a758396d904c9aea964985801c0e9e30fd88f4`；独立下载确认 10 文件闭包、9/9 manifest records、2,408-byte canonical summary（SHA-256 `999a35e25bfa53aaf3ef7f86f7eaf4b596c17b25366ba85cf7193724a41d0b38`）、两个 wheels / 47 members 不含 Parquet/Arrow/Feather/IPC/ORC/CSV/DuckDB/JSONL payload、CycloneDX 1.4 / 8 components、PyArrow 25.0.0、`pip-audit` 0 与 wheel scan `PASS`。

[S7-04 运行手册](runbooks/local-rcaeval-evaluation.md) 冻结许可/下载、唯一命令、缓存/离线复验、回答盲八步顺序、指标判读、失败、安全清理、artifact 审计和全部 17 项 `not_claimed`。这不是完整 upstream benchmark、全数据覆盖、生产准确率、泛化/因果、online/streaming、live remediation、RCAEval Replay、externally timestamped commitment、Cloud/OTel 或统一 Dashboard。S7/P7/P3e/S4/Workflow E 继续 `IN PROGRESS`，Gate E/G5/G2/G4 保持开放，S2-04 继续 `BLOCKED`，P6 保持 `NOT STARTED`；本次文档提交晚于且不等于受测 RC。

### 6.10 S7-05 离线提交证据索引包

**状态**：`DONE`（仅限本节定义的窄切片）。

唯一入口为 `python tools/local-stack/build_submission_bundle.py --offline`。它只读取 `HEAD:docs/evidence/local-submission-evidence.v1.json`，并在 `.local/networkagent-submission` 写出 `submission-index.json`、`limitations.json`、`REPRODUCE.md`、`index.html` 与 `manifest.json` 五个普通文件。输出只含九个历史窄切片的安全元数据、显式未知值、限制和历史引用；不重跑历史验收，不复制历史 artifact payload，也不声明其持续远程可用。tracked source、ledger、ownership、link/hardlink、成员、编码、摘要、隐私或精确重试契约不满足时失败关闭。

受测 RC `67f08d887d6ba6e5ddb4108603af71b19e5e0d0f` 的 [telco-submission run 33405685328](https://github.com/LeegenSteven/NetworkAgent-dev/actions/runs/33405685328) 为 `success` / 33s。Python 3.12 job 99532578035、Python 3.13 job 99532578383、cross-Python equality job 99532644051 与 clean rebuild/publish job 99532668880 分别为 7s/9、10s/9、3s/3、11s/10 success steps。同一 push 的 [telco-local run 33405685291](https://github.com/LeegenSteven/NetworkAgent-dev/actions/runs/33405685291) 也为 `success` / 12m19s；双 Python jobs 99532577952/99532578269 分别为 21/17 success steps，后者四项 release steps 按设计跳过。

唯一 [artifact 9763040737](https://github.com/LeegenSteven/NetworkAgent-dev/actions/runs/33405685328/artifacts/9763040737) 名称为 `networkagent-local-submission-py3.12-attempt-1`，保留 14 天；Python 3.13 和 pull request 不上传。raw ZIP 为 8,759 bytes，outer SHA-256 为 `12e626cd7125f6b04b9f8d817f2db7abadc3c8eba4fca7068909e61355d53295`，inner manifest SHA-256 为 `32a2ffafaab603cd222c6fadf8e4111ecedb26b354d24d8409520c007c4d4e5c`。独立下载确认 exact-five unique regular entries、CRC、四条记录加 manifest、4/4 bytes/SHA，固定 index/limitations SHA-256 为 `741f4cfb29c992e88eaaba0887cdc11373ae536f1683bc86280e6ce5d3594260` / `dd0ca40daeca8d004dafddfee673403f9750bb67a781528721cddd5ec39403f7`，并验证 9 slices/24 runs/66 jobs/9 citations、CSP/四个相对链接、隐私、非闭环与引用政策。

[S7-05 运行手册](runbooks/local-submission-bundle.md) 冻结 source/workspace、exact-five、失败、CI、双摘要与独立审计契约。本 RC 只验收 builder、workflow 与五文件 bundle；九个历史切片仍绑定各自原 RC/runs/jobs/artifacts。S7/P7/P3e/S4/Workflow E 继续 `IN PROGRESS`，P6 继续 `NOT STARTED`，S2-04 继续 `BLOCKED`，Gate E/G2/G4/G5 保持开放；本次文档提交晚于且不等于受测 RC。
...
| 2026-08-31 | 2.0 | RC `b8a9e958a0a3354634f87e2fbc8f76aaf60913dd` 的 Data Lab/Assurance/Container push runs 与显式 Data Lab dispatch 33385881296 全绿。RCAEval 16 个锁定资源 / 五案例完成 aggregate-only 适配、pre-reveal rank/seal/batch commitment、post-reveal 原 seal 复用、隐私输出与离线 exact result；5 ranked / 0 inconclusive，AC@1..5/Average@5/MRR 均为 1,000,000 ppm，ownership validity 为 104,838 ppm（39/372）。唯一 Lab artifact 9755569487 为 148,959 bytes / archive SHA-256 `8afc11102a17310c78e1295a15a758396d904c9aea964985801c0e9e30fd88f4`；独立 10 文件闭包、canonical summary、wheel/SBOM/audit 均通过。 | S7-04 窄切片=`DONE`；P3e/S7/P7/S4/Workflow E=`IN PROGRESS`，Gate E/G5/G2/G4 开放，S2-04=`BLOCKED`，P6=`NOT STARTED`。不声明完整 benchmark、生产准确率、泛化/因果、RCAEval Replay、online/live、外部时间戳、Cloud/OTel/dashboard 或 Gate 关闭；本证据文档提交晚于且不等于受测 RC。 |
| 2026-08-31 | 2.0 | RC `67f08d887d6ba6e5ddb4108603af71b19e5e0d0f` 的 Submission run 33405685328 双 Python build、cross equality 与 clean rebuild/publish 4/4 jobs 全绿；同 push Local run 33405685291 双 jobs 全绿。唯一 artifact 9763040737 为 8,759-byte raw ZIP，outer/inner SHA-256 分别为 `12e626cd7125f6b04b9f8d817f2db7abadc3c8eba4fca7068909e61355d53295` / `32a2ffafaab603cd222c6fadf8e4111ecedb26b354d24d8409520c007c4d4e5c`；独立 exact-five/CRC/manifest、4/4 bytes/SHA、9/24/66/9、CSP/隐私/非闭环/引用审计通过。 | S7-05 窄切片=`DONE`；只验收 builder/workflow/bundle，不重跑或嵌入九个历史切片。S7/P7/P3e/S4/Workflow E=`IN PROGRESS`，P6=`NOT STARTED`，S2-04=`BLOCKED`，Gate E/G2/G4/G5 开放；本证据文档提交晚于且不等于受测 RC。 |
