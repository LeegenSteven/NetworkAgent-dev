...
class EventTranslator:
        ...
        try:
            ...
            if hasattr(adk_event, 'get_function_calls'):               
                ...
                if function_calls:
                    ...
                    async for event in self._translate_function_calls(function_calls, thread_id):
                        yield event
    ...
    async def _translate_text_content(
        ...
        # Check if input is required - this should also end streaming
        input_required = False
        if hasattr(adk_event, 'input_required'):
            input_required = adk_event.input_required
        elif hasattr(adk_event, 'state') and hasattr(adk_event.state, 'input_required'):
            input_required = adk_event.state.input_required
        elif hasattr(adk_event, 'status') and hasattr(adk_event.status, 'state'):
            # Handle TaskStatusUpdateEvent case
            from a2a.types import TaskState
            input_required = adk_event.status.state == TaskState.input_required
        
        # Handle None values: if is_final_response=True or input_required=True, it means streaming should end
        should_send_end = (is_final_response or input_required) and not is_partial
        
        logger.info(f"📥 Text event - partial={is_partial}, turn_complete={turn_complete}, "
                    f"is_final_response={is_final_response}, input_required={input_required}, should_send_end={should_send_end}, "
                    f"currently_streaming={self._is_streaming}")

        if is_final_response or input_required:
            ...
            if input_required:
                logger.info("⏭️ Skipping input required event (ending stream to wait for user input)")
            else:
                logger.info("⏭️ Skipping final response event (content already streamed)")
            
            # If we're currently streaming, this final response or input required means we should end the stream
            if self._is_streaming and self._streaming_message_id:
                ...
                if input_required:
                    logger.info(f"📤 TEXT_MESSAGE_END (from input required): {end_event.model_dump_json()}")
                else:
                ...
                if input_required:
                    logger.info("🏁 Streaming completed via input required")
                else:
    ...
    async def translate_lro_function_calls(self, adk_event: ADKEvent, thread_id: str)-> AsyncGenerator[BaseEvent, None]:
            ...
            for i, part in enumerate(adk_event.content.parts):
                if part.function_call:
                    if not long_running_function_call and part.function_call.id in (
                        ...
                        # Encode thread_id in the tool_call_id for proper routing when tool result comes back
                        # Format: original_id::thread_id
                        original_tool_call_id = long_running_function_call.id
                        tool_call_id = f"{original_tool_call_id}::{thread_id}"
    ...
    async def _translate_function_calls(
        ...
        Yields:
            Tool call events (START, ARGS, END) - only for user-facing tools, not internal supervisor tools
        """
        logger.debug("translating function calls")
        logger.debug(function_calls)
        # List of internal supervisor tools that should not emit tool call events to UI
        ...
        for func_call in function_calls:
            original_tool_call_id = getattr(func_call, 'id', str(uuid.uuid4()))
            tool_name = getattr(func_call, 'name', '')
            
            # Skip internal supervisor tools - their calls should be processed by the LLM, not sent to UI
            if tool_name in internal_supervisor_tools:
                logger.debug(f"Skipping tool call events for internal supervisor tool: {tool_name} (id: {original_tool_call_id})")
                continue
    ...
    async def _translate_function_response(
        ...
        for func_response in function_response:
            ...
            tool_name = getattr(func_response, 'name', '')
            
            # Skip internal supervisor tools - their results should be processed by the LLM, not sent to UI
            if tool_name in internal_supervisor_tools:
